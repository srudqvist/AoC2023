...
def part1(data, width, height, numberList):
    numbers = [
        ...
        for n in numberList
        if any(
            [isSymbol(data[srow][scol]) for srow, scol in getAdjacent(height, width, n)]
        )
    ]
...
def part2(data, width, height, numberList):
    ...
    print(
        "Part 2:",
        sum([x[0][3] * x[1][3] for x in [v for v in starDict.values() if len(v) == 2]]),
    )
...
def extractNumbers(line, row):
    return [
        (row, x[1], len(x[0]), int(x[0]))
        for x in zip(
            re.findall(r"\d+", line), [x.start() for x in re.finditer(r"\d+", line)]
        )
    ]
...
def getAdjacent(height, width, number):
    ...
    return [
        x
        for x in adjacentCoords
        if x[0] >= 0 and x[0] < height and x[1] >= 0 and x[1] < width
    ]
...
if __name__ == "__main__":
    ...
    numberList = list(
        chain(*[extractNumbers(line, row) for row, line in enumerate(data)])
    )
